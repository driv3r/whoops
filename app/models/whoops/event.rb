module Whoops
  class Event
    ...
    def self.record(params)
      ...
      event_group_params                    = params.slice(*EventGroup.field_names)
      event_group_params[:last_recorded_at] = params[:event_time]
      event_group_params
      event_group = EventGroup.handle_new_event(event_group_params)

      event_params = params.slice(*Event.field_names)
      event_group.events.create(event_params)
    end

    def self.search(query)
      conditions = MongoidSearchParser.new(query).conditions
      where(conditions)
    end
