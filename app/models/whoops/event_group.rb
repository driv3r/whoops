  ...
  class EventGroup
    ...
    class << self
      def handle_new_event(params)
        identifying_params = params.slice(*EventGroup.identifying_fields)
        event_group = EventGroup.first(:conditions => identifying_params)

        if event_group
          event_group.attributes = params
        else
          event_group = EventGroup.new(params)
        end
    ...
    def send_notifications
      ...
      matcher = NotificationRule::Matcher.new(self)
      NotificationMailer.event_notification(self, matcher.matches).deliver unless matcher.matches.empty?
      self.notify_on_next_occurrence = false
    end
  end
end
