module Whoops
  module EventGroupsHelper
    def event_group_scoped_link(event_group, scope)
      ...
      link_to(event_group.send(scope), event_groups_path(new_filter))
    end

    # meant for consumption by options_from_collection_for_select
    def filter_options
      all_event_groups = EventGroup.all
      return @filter_options if @filter_options

      @filter_options = Hash.new{|h, k| h[k] = []}

      @filter_options["service"] = EventGroup.services.to_a

      all_event_groups.each do |event_group|
        mundane_fields = (Filter.field_names & EventGroup.field_names) - ["service", "message"]
